...
Where the report earns belief. Load when placing a number, a graph, a
source, or any claim that has to survive a suspicious reader.
...
70 percent, so it has little room to absorb a new contract.
...
- Plot two or three variables at most. A graph illustrates a point; it is
  not a repository of data.
- Place it after the text that discusses it, as close as possible, ideally
  on the same page.

Bad: a chart titled "Quarterly Revenue" with six overlaid product lines and
no labels on the gridlines.
Good: a chart titled "Revenue growth stalled in Q3" with two lines, this
year against last, every axis labeled.

Pick the form from the job:

- Line for a trend over a continuous scale.
- Bar for comparison of discrete things.
- Pie for parts of a known whole (100 percent) only. A pie cannot show time,
  change, or unrelated items.

Bad: a pie chart of revenue across five years.
Good: a line chart of revenue across five years; a pie only for this year's
split by segment.
...
- The startling and the contested. A claim that will make the reader stop
  needs its source right there.
- Anything whose source reliability is itself in question.
...
reader's stride and cost more understandability than they buy.

Bad: Sales rose 12 percent.[footnote to your own calculation]
Good: Sales rose 12 percent, against the 4 percent the trade association
reported for the sector.[footnote to the association's figure]

## Source credibility

Use a variety of source KINDS, not just a number of sources. Each kind has a
weakness; another kind's strength covers it. Three sources of one kind is one
kind's blind spot, tripled.
...
- Subjective matters (management quality, how a firm treats its people) come
  from periodicals, not from the subject.

Bad: The firm leads its industry on safety (source: the firm's About page).
Good: The firm reports zero lost-time incidents (its own filing), though a
2024 trade-press investigation documented three unreported near-misses.

## Argument structure

Every supported claim has four parts. Name them, and the weak one shows:

- Claim: what you assert.
- Evidence: the facts under it.
...
Good: Turnover is 30 percent, triple the sector's 10 percent, and exit
interviews name management in two of every three cases; morale is a likelier
cause than the labor market.

### The five-part evidence test

Evidence must pass all five, not a majority:

- Authoritative: from a source with real standing on the point.
- Accurate: correct.
- Precise: exact enough to carry the claim, and no vaguer.
- Relevant: bears on this claim, not a neighbouring one.
- Sufficient: enough of it for the weight placed on it.

One anecdote can be authoritative, accurate, precise, and relevant, and still
fail on sufficient. Check all five before the claim ships.

### The fallacy check

Run before shipping. Aimed at the fallacies an agent-drafted report actually
...
- Non sequitur: the conclusion does not follow from the facts given.
- Faulty analogy: two things treated as alike in all relevant ways when they
  are not.
- False dilemma: two options offered when more exist.
- Slippery slope: a small step asserted to force an inevitable bad end.
...
Every opinion is adjudicated before it enters the report. Three outcomes, and
you must pick one:

- Accept it as fact, when it is verifiable and you verified it.
- Include it as opinion, labeled as someone's judgment, never stated as
  information.
- Reject it.
...
overlap in the two sales forces. The overlap is documented; the conclusion
is her opinion.
...
Per-claim citation, inline or as a footnote:
    ...
    <author or issuing body>, <title>, <date>, <locator: page, section, or
    URL>. <one-clause descriptor: what kind of source, and its known slant>.

Worked:

    Acme Corp, Form 10-K FY2024, filed 2025-02-14, p. 47. Audited filing;
    carries litigation the company would not otherwise disclose.

End-of-report source summary, one row per source:

    <source> | <kind> | <strength for this report> | <weakness>

Worked:

    Acme 10-K       | audited filing | hard figures, disclosed litigation |
                      a year old, backward-looking
    Trade Weekly    | periodical     | current, independent | one reporter,
    2025-06                                                   no second
                                                              confirmation
