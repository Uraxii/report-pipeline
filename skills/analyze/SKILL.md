...
Operator's per-question instruction outranks every rule here. Instruction
differs from anything below -> instruction wins. Following instruction against
a rule here = deviation, and handover section 12 records it. Record's three
fields sit below the handover contract.
...
Makes artifacts a writer works from. No report format, no audience.

## The handover contract

Every run hands over these twelve, or names the ones that do not apply and
says why. A section quietly missing is the failure this contract exists to
catch.

1. **Problem frame.** The environment, the actors and their conflicts, the
   three-part problem statement, the same question cast as who, what, when,
   why and how, who approved it or that approval was not sought, the terms the
   analysis defines, the itemized constraints, the problem's structure class,
   and the depth this question earned.
2. **Fact notes.** One row per fact, in your own words, with its source and
   whether it is fact, assumption, or judgment. Open until step 7 closes;
   facts surfaced during per-option analysis go here, not into a second list.
3. **Source assessment.** One row per source: what it is, its strengths, its
   weaknesses, and which judgments it carries.
4. **Assumptions register.** Each assumption, why it is held, and what follows
   if it is wrong. Critical ones marked.
5. **Criteria set, marked frozen.** Screening criteria with their pass or fail
...
10. **Method log.** Two parts, kept apart. Escalation methods: which ran and
    ...
    output.
11. **Recommendation.** The thesis in one sentence, written after the fact
    notes and never before them. One committed course. The matrix result in a
    paragraph a reader need not open an attachment to understand. The margin,
...
Seven clauses bind across all twelve.

- Every section appears at every depth tier. Only its form changes. Cheap
  tier: a section is a table, a row set, or a line. Prose only where the
  contract above names a paragraph: the environment in section 1, the matrix
  result in section 11. Method escalated -> its section expands into prose
  only for output no table carries. Section written as paragraphs when a table
  would hold it = padding.
- Likelihood and confidence never share a sentence. One says how likely the
  thing is. Other says what the evidence behind that number is worth.
- Criteria are frozen before any option exists and take no new member
  afterwards. Criterion added once options are on the table = how a favourite
  gets rescued.
- Every option is measured on its own before any two are compared. Compare
  while analyzing -> winner out of a field of bad options.
- The recommendation is one course, worded so the decision-maker has only to
  approve or disapprove.
- Order of thinking is not order of writing. Never let the eventual write-up's
  shape pull work forward.
- A judgment that is not empirical carries no likelihood band. No observation
  could confirm or disconfirm it: it rests on a definition, a policy, a legal
  or ethical standard, or a value the authorizing party set. Percentage on one
  = invented number. Write the premise as a premise, name where the premise
  comes from, and in place of a falsifier name what would have to change for
  the judgment to stop holding. Confidence still applies and still gets its own
  sentence. That shape is what check 10 passes on for such a judgment; a band
  is not.

## When a fix is barred

Step 9 says a failed check is a fix, not a note. Two situations bar the fix,
only these two. Operator's instruction overrides the check, under the
Meta-rule. Or two rules in this skill collide and no rewriting satisfies both.
Merely expensive is not barred.
...
Deviation missing a field = unrecorded fail. Material the research never
...
- Do not present a technique's output as proof the judgment is right. Running
  ...
  accuracy.
- Do not trigger a challenge technique yourself. Competing hypotheses, Team A
  and Team B, devil's advocacy and red team run only when the operator names
...
Steps 1 and 2 are separate and ordered. Collapse them and the constraints stop
being checkable.
...
3. **Size and route.** Scale the work to the problem, not to the format. Name
   ...
   An escalation method runs only on a trigger in the question. The tiers and
   the routing table are in references/framing.md.
4. **Evidence.** Build the fact notes, the source assessment and the
   assumptions register. Run the key assumptions check here, against the
   answer the question already presumes and before the analysis hardens, and
   record its output. It runs twice: here and at step 8. Where step 3 routed
   to probability, derive it from a base rate first. references/evidence.md.
5. **Criteria.** Screening criteria as a pass or fail gate, then evaluation
   criteria to separate the survivors. Five parts each, weights summing to 100
   percent, and one line of rationale per weight. Fix direction and
   normalization before any score is written. Then freeze the set. The freeze
   binds criteria only; the fact notes stay open. references/criteria.md.
6. **Options.** At least two genuinely distinct options, or a written defense
   of why no alternative exists. Where nobody handed you alternatives, build
   them. Run the screening gate and record a verdict per option.
...
9. **Check.** Run references/handover-check.md against the assembled handover.
   ...
   step the check names and produce the missing thing. The verdicts go into
...
- **Two options versus one real option.** Demand is two genuinely distinct
  ...
  comparison: one real option plus one bad one -> false winner. No second real
  option exists -> write the single-option defense, say the matrix does not
  apply. Never fabricate.
- **Ill-structured problem versus one committed course.** Package of measures
  is ONE course, named as one, parts ordered. Ruling in full at
  references/comparison.md, "Commit to one course".
- **Close call versus one committed course.** Still name one, disclose the
  margin, hand the runner-up over alongside. Ruling in full at
  references/comparison.md, "Commit to one course".
- **Empty field versus one committed course.** Screening can kill every
  ...
  sequenced package is, and hand that over as the recommendation. Several
  changes would each work -> take the one you judge cheapest to reach, one
  line on why the others lose. Matrix does not apply, and section 8 says so
  for the same reason the single-option defense does. Never soften a screen to
  keep an option alive, never score a screened-out option, never hand the
  empty field over on its own: field with no course attached = "recommend
  further study" under another name.
- **One root cause versus the single-cause fallacy.** Naming exactly one root
  cause is correct. Stopping at the first uncorroborated cause is the fallacy.
  Different failures. Single root-cause label earned only after contributing
  causes were searched for and each accepted cause corroborated twice.

## Stated limits

Holes, not features. Named so nothing gets quietly asserted.

Weighted total is plain arithmetic: multiply each score by its criterion's
weight, add across. Arithmetic, not a sourced method.

Three different things called sensitivity. Not interchangeable, each has one
home. Evidence-sensitivity: references/evidence.md. Weight re-run:
references/comparison.md. Cost sensitivity: references/method-cost.md. Run one,
claim another done -> commonest way this handover ships incomplete.
...
- Calibration over time not claimed: references/evidence.md, "Deriving a
  probability".
...
- references/framing.md. Steps 1 to 3: environment, actors, three-part problem
  statement, constraints list, problem structure, depth tiers, technique
  routing table.
- references/evidence.md. Step 4 plus the uncertainty layer used throughout:
  fact notes, source assessment, assumptions, deriving a probability,
  likelihood ladder, confidence, falsifiers, key assumptions check,
  evidence-sensitivity test.
- references/criteria.md. Step 5: screening categories, five parts of an
  evaluation criterion with a worked example, weights and their rationale,
  direction, normalization, mandatory gate, freeze.
- references/options.md. Steps 6 and 7: distinctness, building options when
  none were handed over, single-option defense, per-option analysis,
  alternative-futures test.
- references/comparison.md. Step 8: matrix with a worked example, both totals,
  weight re-run, tie-breaking, close-call clause, committing to one course,
  handing over the losers.
...
- references/method-risk.md, references/method-root-cause.md,
  references/method-causal-claims.md, references/method-cost.md. Escalation
